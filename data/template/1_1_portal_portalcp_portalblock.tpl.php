...
0
|| checktplrefresh('./template/default/portal/portalcp_portalblock.htm', './template/default/portal/portalcp_pageblock.htm', 1388020721, '1', './data/template/1_1_portal_portalcp_portalblock.tpl.php', './template/default', 'portal/portalcp_portalblock')
|| checktplrefresh('./template/default/portal/portalcp_portalblock.htm', './template/default/portal/portalcp_pageblock.htm', 1388020721, '1', './data/template/1_1_portal_portalcp_portalblock.tpl.php', './template/default', 'portal/portalcp_portalblock')
|| checktplrefresh('./template/default/portal/portalcp_portalblock.htm', './template/default/portal/portalcp_nav.htm', 1388020721, '1', './data/template/1_1_portal_portalcp_portalblock.tpl.php', './template/default', 'portal/portalcp_portalblock')
...
<?php if($_G['setting']['portalstatus'] ) { ?>模塊管理<?php } ?>
</div>
</div>
...
<?php } if(!empty($blocks)) { ?>
<h3 class="ptn pbn">請在下面選擇要推送的模塊</h3>
...
<option value="">全部頁面</option><?php if(is_array($tpls)) foreach($tpls as $tpl => $tplname) { ?><option value="<?php echo $tpl;?>"<?php if($tpl == $_GET['targettplname']) { ?>selected<?php } ?>><?php echo $tplname;?></option>
<?php } ?>
</select>&nbsp;
<?php } ?>
模塊標識:
...
<label for="showrecommendtip"><input type="checkbox" name="showrecommendtip" id="showrecommendtip" class="pc" fwin="mods" value="1">顯示推送標誌</label>&nbsp;
...
<p>
僅顯示搜索條件下的模塊。 <a href="portal.php?mod=portalcp&amp;ac=portalblock" class="xi2">點擊返回列表頁</a>
</p>
...
<?php } else { ?>
<li<?php if($op === 'getblocklist') { ?> class="a"<?php } ?>><a href="portal.php?mod=portalcp&amp;ac=portalblock&amp;op=getblocklist">模塊列表</a></li>
<?php } ?>
<li<?php if($op === 'verifydata') { ?> class="a"<?php } ?>><a href="portal.php?mod=portalcp&amp;ac=portalblock&amp;op=verifydata<?php echo $addurl;?>">未審核數據</a></li>
<li<?php if($op === 'verifieddata') { ?> class="a"<?php } ?>><a href="portal.php?mod=portalcp&amp;ac=portalblock&amp;op=verifieddata<?php echo $addurl;?>">已審核數據</a></li>
<?php if($_GET['searchkey'] && $_block) { ?>
<li class="o"><a href="portal.php?mod=portalcp&amp;ac=portalblock&amp;op=getblocklist">返回模塊列表</a></li>
<?php } ?>
</ul>
...
<td><?php if($value['stickgrade']) { ?>置頂 <?php echo $value['stickgrade'];?><?php } else { ?>否<?php } ?></td>
<td>
<a href="portal.php?mod=portalcp&amp;ac=block&amp;op=managedata&amp;bid=<?php echo $value['bid'];?>&amp;dataid=<?php echo $value['dataid'];?><?php if($_GET['from']) { ?>&amp;from=<?php echo $_GET['from'];?><?php } ?>" onclick="showWindow('showblock', this.getAttribute('href'));">編輯</a>
</td>
</tr>
<?php } ?>
<tr>
...
<td>
<a href="portal.php?mod=portalcp&amp;ac=block&amp;op=verifydata&amp;bid=<?php echo $value['bid'];?>&amp;dataid=<?php echo $value['dataid'];?>" onclick="showWindow('verifydata', this.href, 'get', 0)" class="xi2">審核</a>
</td>
</tr>
<?php } ?>
<tr>
<td colspan="7">
<label for="chkall" onclick="checkall(this.form, 'ids')"><input type="checkbox" name="chkall" id="chkall" class="pc" />全選</label>
<label for="op_delete"><input id="op_delete" class="pr" value="delete" name="optype" type="radio">刪除</label>
<label for="op_pass"><input id="op_pass" class="pr" value="pass" name="optype" type="radio">通過</label>
...
<a href="portal.php?mod=portalcp&amp;ac=block&amp;op=block&amp;bid=<?php echo $block['bid'];?>" target="_blank">屬性</a>
<?php } else { ?>
&minus;
<?php } if($block['perm']['allowdata']) { ?>
<a href="portal.php?mod=portalcp&amp;ac=block&amp;op=data&amp;bid=<?php echo $block['bid'];?>" target="_blank">數據</a>
<?php } else { ?>
&minus;
<?php } if($block['isrecommendable']) { ?>
<a href="portal.php?mod=portalcp&amp;ac=portalblock&amp;op=verifydata&amp;searchkey=<?php echo $block['bid'];?>">審核</a>
<a href="portal.php?mod=portalcp&amp;ac=portalblock&amp;op=verifieddata&amp;searchkey=<?php echo $block['bid'];?>">推送庫</a>
<?php } else { ?>
&minus;
&minus;
<?php } ?>
...
<tr>
<td>
<label for="chkall"><input type="checkbox" name="chkall" id="chkall" class="pc" onclick="checkall(this.form, 'bids')" />全選</label>
</td>
<td colspan="5">
<input type="hidden" name="<?php echo $op;?>submit" value="1" />
<input type="hidden" name="formhash" value="<?php echo FORMHASH;?>" />
<button type="submit" value="true" name="submit" class="pn"><em>加入更新隊列</em></button>
<img src="<?php echo IMGDIR;?>/faq.gif" alt="Tip" class="vm" onmouseover="showTip(this)" tip="已經加入更新隊列的模塊並不立即更新，當此模塊所在的頁面被訪問時才觸發更新，請耐心等待" />
</td>
</tr>
<?php } } else { ?>
<tr><td>沒有相關數據</td></tr>
