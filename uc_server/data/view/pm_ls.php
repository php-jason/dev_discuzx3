...
<?php if($pmlist) { ?>
	<tr class="ctrlbar">
		...
		<td class="ava"><button name="pmsend" type="submit">刪除</button></td>
		<td class="pef"></td>
		<td class="">
			...
			<?php } ?>
				...
				<?php if($pm['pmtype'] == 1) { ?>
					<?php if($pm['lastauthorid'] == $user['uid']) { ?>
						發起者<?php echo $pm['tousername'];?>
					<?php } else { ?>
						來自: <?php echo $pm['lastauthor'];?>
					<?php } ?>
				...
				<h2>
				<?php if($pm['pmtype'] == 1) { ?>
					<a href="index.php?m=pm_client&a=view&touid=<?php echo $pm['touid'];?>&daterange=<?php echo $pm['daterange'];?>&filter=<?php echo $pm['filter'];?>&scroll=bottom&<?php echo $extra;?>" id="pm_view_<?php echo $pm['pmid'];?>">與<?php echo $pm['tousername'];?>的對話</a>
				<?php } else { ?>
					<a href="index.php?m=pm_client&a=view&plid=<?php echo $pm['plid'];?>&daterange=<?php echo $pm['daterange'];?>&filter=<?php echo $pm['filter'];?>&scroll=bottom&<?php echo $extra;?>" id="pm_view_<?php echo $pm['pmid'];?>"><?php echo $pm['subject'];?></a>(參與人數: <?php echo $pm['members'];?>)
				<?php } ?>
				</h2>
	...
	<?php } ?>
		...
		<tfoot>
			<tr class="ctrlbar">
				...
				<td class="ava"><button onclick="this.form.pmsend.click()" type="button">刪除</button></td>
				<td class="pef"></td>
				<td class="">
					<?php if($multipage) { ?><?php echo $multipage;?><?php } ?>
				</td>
