...
<div class="ucnav">
<a <?php if($filter == 'newpm') { ?>class="ucontype" <?php } ?>href="index.php?m=pm_client&a=ls&filter=newpm">未讀消息<?php if($unreadpmnum) { ?><strong>[<?php echo $unreadpmnum;?>]</strong><?php } ?></a>
...
<a <?php if($folder == 'send') { ?>class="ucontype sendpmontype" <?php } else { ?> class="sendpm" <?php } ?>href="index.php?m=pm_client&a=send&folder=send">發送短消息</a>

<?php if($unreadpmnum) { ?>
	<span class="navinfo">
		<img src="images/default/newpm.gif" />
		<?php if($unreadpmnum) { ?><strong><?php echo $unreadpmnum;?></strong> <a <?php if($filter == 'newpm') { ?>class="ontype" <?php } ?>href="index.php?m=pm_client&a=ls&filter=newpm">條未讀消息</a><?php } ?>
	</span>
<?php } ?>
</div>